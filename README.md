...
> ⚠️ **开发阶段说明**：本项目目前处于早期开发阶段，仅支持 **肿瘤相关诊断（ICD-10 C 开头编码，如 C50、C34 等）** 的编码映射，暂不支持其他疾病类别的 ICD 编码。后续版本将逐步扩展覆盖范围。
...
| 项目 | 说明 |
|------|------|
| 支持编码 | **仅肿瘤相关 ICD-10 编码（C00-D49）** |
| 支持诊断类型 | 恶性肿瘤、原位癌、良性肿瘤、动态未定肿瘤等 |
| 不支持 | 非肿瘤诊断（如循环、呼吸、消化系统等疾病） |
| 后续计划 | 逐步扩展至全类别 ICD-10 编码 |
